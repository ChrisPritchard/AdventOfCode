...
let input = System.IO.File.ReadAllLines "Day22-input.txt"
...
let parseNodeLine line =
    let s = splits ["/dev/grid/node-x";"-y";" ";"T";"%"] line
    { x = int s.[0]
      y = int s.[1]; 
      size = int s.[2]
      used = int s.[3] }
...
let part1 () = 
    nodes 
    |> Array.filter (fun n -> n.used > 0)
    |> Array.collect (fun n -> 
        nodes 
        |> Array.filter (fun o -> o <> n && o.size - o.used >= n.used) 
        |> Array.map (fun o -> n, o))
    |> Array.length
