...
let avalid, bvalid = 4L, 8L

let part2 () =
    let _, _, count =
        ((astart, bstart, 0), [1..5000000]) 
        ||> List.fold (fun (aprev, bprev, count) _ -> 
            let mutable anext, bnext = next afactor aprev, next bfactor bprev
            while anext % avalid <> 0L do
                anext <- next afactor anext
            while bnext % bvalid <> 0L do
                bnext <- next bfactor bnext
            if isMatch (anext, bnext) 
            then anext, bnext, count + 1
            else anext, bnext, count)
    count