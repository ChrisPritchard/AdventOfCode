...
let evolvedMap = map |> Seq.map (fun (x, y) -> (x, y), Infected) |> Map.ofSeq

let part2 () =

    let rec evolvedCrawler x y dx dy grid count iterations =
        if iterations = 0 then count
        else
            let nodeState = Map.tryFind (x, y) grid |> Option.defaultValue Clean
            let (dx, dy), grid, count = 
                match nodeState with
                | Clean -> 
                    turnLeft dx dy, Map.add (x, y) Weakened grid, count
                | Weakened -> 
                    (dx, dy), Map.add (x, y) Infected grid, count + 1
                | Infected -> 
                    turnRight dx dy, Map.add (x, y) Flagged grid, count
                | Flagged -> 
                    (dx * -1, dy * -1), Map.add (x, y) Clean grid, count
            let x, y = x + dx, y + dy
            evolvedCrawler x y dx dy grid count (iterations - 1)

    evolvedCrawler 0 0 0 -1 evolvedMap 0 10000000