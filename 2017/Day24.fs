...
let input = System.IO.File.ReadAllLines "./inputs/day24.txt"

let components = input |> Array.map (split "/" >> fun a -> int a.[0], int a.[1]) |> List.ofArray

let part1 () =

    let marked = List.indexed components

    let rec expand (edge, sofar, remaining) =
        let next = 
            remaining 
