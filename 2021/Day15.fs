...
type PriorityQueue(maxSize, comparer) =
    // actually just a binary heap, but same diff
...
let totalRisk ty tx getRisk =
    ...
    let openSet = PriorityQueue(width * height, fun (x1, y1) (x2, y2) -> fScores[x1][y1] - fScores[x2][y2])
    openSet.Enqueue (0, 0)
...
let part1 () =
    totalRisk (height - 1) (width - 1) (fun y x -> processed[y][x])
...
let part2 () =
    ...
    totalRisk (height * 5 - 1) (width * 5 - 1) getRisk
