...
let winning_counts time record = 
    [|0..time - 1|] |> Array.filter (fun time_held -> (time - time_held) * time_held > record) |> Array.length

let total = races |> Array.map (fun (time, record) -> winning_counts time record |> int64) |> Array.reduce (*)
...
let win_chances = [|0L..longer_time - 1L|] |> Array.filter (fun time_held -> (longer_time - time_held) * time_held > longerRecord) |> Array.length
